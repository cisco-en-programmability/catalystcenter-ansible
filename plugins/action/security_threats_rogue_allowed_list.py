...
required_if = [
    ("state", "present", ["macAddress"], True),
    ("state", "present", ["payload"], True),
    ("state", "absent", ["macAddress"], True),
...
class SecurityThreatsRogueAllowedList(object):
    ...
    def exists(self):
        ...
        o_id = (
            ...
            or requested_obj.get("macAddress")
        )
    ...
    def create(self):
        result = self.catalystcenter.exec(
            family="devices",
            function="add_allowed_mac_address",
    ...
    def delete(self):
        ...
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("macAddress")
            if id_:
                self.new_object.update(dict(mac_address=id_))
        result = self.catalystcenter.exec(
            family="devices",
            function="remove_allowed_mac_address",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = SecurityThreatsRogueAllowedList(self._task.args, catalystcenter)
