...
class SitesWirelessSettingsSsids(object):
    ...
    def get_all_params(self, name=None, id=None):
        ...
        new_object_params["wlan_type"] = self.new_object.get("wlanType") or self.new_object.get("wlan_type")
        new_object_params["auth_type"] = self.new_object.get("authType") or self.new_object.get("auth_type")
        new_object_params["l3auth_type"] = self.new_object.get("l3authType") or self.new_object.get("l3auth_type")
    ...
    def get_object_by_id(self, id):
        result = None
        try:
            items = self.catalystcenter.exec(family="wireless", function="get_ssid_by_id", params={"id": id})
            if isinstance(items, dict):
                if "response" in items:
                    items = items.get("response")
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = SitesWirelessSettingsSsids(self._task.args, catalystcenter)
