...
class ComplianceDevice(object):
    ...
    def get_all_params(self, name=None, id=None):
        new_object_params = {}
        new_object_params['compliance_status'] = self.new_object.get('complianceStatus') or \
            self.new_object.get('compliance_status')
        new_object_params['device_uuid'] = self.new_object.get('deviceUuid') or \
            self.new_object.get('device_uuid')
    ...
    def get_object_by_id(self, id):
        ...
        try:
            items = self.catalystcenter.exec(
                ...
                params={"device_uuid": id}
            )
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = get_dict_result(items, 'device_uuid', id)
        except Exception:
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get("device_uuid")
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("deviceUuid")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters("The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(device_uuid=_id))
            if _id:
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = ComplianceDevice(self._task.args, catalystcenter)
