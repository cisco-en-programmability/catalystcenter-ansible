...
required_if = [
    ("state", "present", ["discoveryId", "id", "jobId"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class DiscoverysJobs(object):
    ...
    def get_all_params(self, name=None, id=None):
        new_object_params = {}
        new_object_params["job_id"] = self.new_object.get(
            "jobId"
        ) or self.new_object.get("job_id")
        new_object_params["limit"] = self.new_object.get("limit")
        new_object_params["offset"] = self.new_object.get("offset")
        new_object_params["order_by"] = self.new_object.get(
            "orderBy"
        ) or self.new_object.get("order_by")
    ...
    def get_object_by_name(self, name):
        ...
        try:
            items = self.catalystcenter.exec(
                family="devices",
                function="fetches_all_the_discovery_job_details_by_discovery_id",
    ...
    def get_object_by_id(self, id):
        ...
        try:
            items = self.catalystcenter.exec(
                family="devices",
                function="fetches_the_discovery_job_details_for_the_given_job_id",
                params={"job_id": id},
            )
            if isinstance(items, dict):
                if "response" in items:
                    items = items.get("response")
            result = get_dict_result(items, "job_id", id)
        except Exception:
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get("job_id")
        ...
        if name_exists:
            ...
            if _id:
                ...
                self.new_object.update(dict(job_id=_id))
            if _id:
    ...
    def create(self):
        result = self.catalystcenter.exec(
            family="devices",
            function="starts_the_existing_discovery",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = DiscoverysJobs(self._task.args, catalystcenter)
