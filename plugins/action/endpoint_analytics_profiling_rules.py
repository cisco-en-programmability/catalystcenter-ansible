...
class EndpointAnalyticsProfilingRules(object):
    ...
    def get_object_by_id(self, id):
        ...
        try:
            items = self.catalystcenter.exec(
                family="ai_endpoint_analytics", function="get_details_of_a_single_profiling_rule", params={"rule_id": id}
            )
            if isinstance(items, dict):
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = EndpointAnalyticsProfilingRules(self._task.args, catalystcenter)
