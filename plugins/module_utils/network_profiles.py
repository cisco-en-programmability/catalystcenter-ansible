...
import time
from ansible_collections.cisco.dnac.plugins.module_utils.dnac import CatalystCenterBase
...
class NetworkProfileFunctions(CatalystCenterBase):
    ...
    def check_site_template(self, each_profile, profile_info):
        ...
        self.log(
            "Check given template exist in the Catalyst Center for {0}, {1}".format(
        ...
        self.log(
            "Fetching onboarding template details for: {0}".format(
        ...
        self.log(
            "Fetching day-N template details for: {0}".format(day_n_templates), "DEBUG"
        )
        ...
        if site_names:
            ...
            for site in site_names:
                if site:
                    site_data = {}
                    site_data["site_exist"], site_data["site_id"] = self.get_site_id(
                        site
                    )
                    site_data["site_names"] = site
                    validated_sites.append(site_data)
                    self.log(
                        "Site '{0}' existence: {1}, Site ID: {2}".format(
                            site, site_data["site_exist"], site_data["site_id"]
                        ),
                        "INFO",
                    )
                    ...
                    if child_sites:
                        self.log(
                            "Child sites found for '{0}': {1}".format(
                                site, child_sites
                            ),
                            "DEBUG",
                        )
                    ...
                    if not site_data["site_exist"]:
                        self.msg = "Given site {0} not exist in Catalyst Center".format(
                            site
                        )
            ...
            if len(validated_sites) > 0:
                ...
                unique_sites = [
                    dict(final_site)
                    for final_site in {
                        frozenset(site.items()) for site in validated_sites
                    }
                ]
                self.log(
                    "Found Site ID(s) list: {0}".format(self.pprint(unique_sites)),
                    "INFO",
                )
    ...
    def get_child_sites(self, site_name_hierarchy):
        ...
        self.log(
            "Fetching child sites for '{}'. Request parameters: {}".format(
                site_name_hierarchy, get_sites_params
            ),
            "DEBUG",
        )
        ...
        try:
            ...
            if response and isinstance(response, dict):
                ...
                self.log(
                    "Found {0} child sites for site area: '{1}'".format(
                ...
                if not child_sites:
                    self.log(
                        "No child sites found for '{0}'. Returning None.".format(
                            site_name_hierarchy
                        ),
                        "DEBUG",
                    )
                ...
                for child in child_sites:
                    ...
                    self.log(
                        "Received child site: '{0}' with ID: '{1}'".format(
                    ...
                    if child_site_id and child_site_name_hierarchy:
                        self.log(
                            "Processing child site: '{0}' (ID: '{1}')".format(
                                child_site_name_hierarchy, child_site_id
                            ),
                            "DEBUG",
                        )
                ...
                self.log(
                    "All child sites for site area: '{0}': {1}".format(
                        site_name_hierarchy, self.pprint(child_site_response)
                    ),
                    "DEBUG",
                )
    ...
    def get_templates_details(self, template_list):
        ...
        try:
            ...
            self.log(
                "Response from gets_the_templates_available API: {0}".format(
            ...
            if not response or not isinstance(response, list):
                self.log(
                    "Invalid or empty response received. No templates found.", "WARNING"
                )
                return None
            ...
            for input_template in template_list:
                ...
                for each_template in response:
                    if each_template.get("name") == input_template:
                        ...
                        self.log(
                            "Template '{0}' found with ID '{1}'.".format(
                                input_template, each_template.get("templateId")
                            ),
                            "DEBUG",
                        )
                        break
                ...
                if not template_info:
                    self.msg = (
                        "Given template '{0}' does not exist in Catalyst Center".format(
                            input_template
                        )
                    )
                    self.log(self.msg, "ERROR")
            ...
            if len(template_info_list) > 0:
                self.log(
                    "Final validated template details: {0}".format(
                        self.pprint(template_info_list)
                    ),
                    "INFO",
                )
    ...
    def get_network_profile(self, profile_type, offset, limit):
        ...
        param = {"type": profile_type, "offset": offset, "limit": limit}
        self.log(
            "Fetching network profiles for profile type: {0}".format(profile_type),
            "INFO",
        )
        ...
        try:
            ...
            self.log(
                "Response from retrieves_the_list_of_network_profiles_for_sites API: {0}".format(
                    self.pprint(response)
                ),
                "DEBUG",
            )
            ...
            if not profiles or not isinstance(profiles, list):
                self.log(
                    "Invalid or missing network profile response, expected dict but got {0}".format(
            ...
            self.log(
                "Received network profile response: {0}".format(self.pprint(profiles)),
                "INFO",
            )
            return profiles
    ...
    def get_templates_for_profile(self, profile_id):
        ...
        """
        self.log(
            "Fetching CLI templates for profile ID: {0}".format(profile_id), "INFO"
        )
        ...
        try:
            response = self.execute_get_request(
                "network_settings",
                "retrieve_cli_templates_attached_to_a_network_profile",
                param,
            )
            self.log(
                "Response from retrieve_cli_templates_attached_to_a_network_profile "
                + "API: {0}".format(self.pprint(response)),
                "DEBUG",
            )
            ...
            if not templates or not isinstance(templates, list):
                self.log(
                    "Invalid or missing template response, expected list but got {0}".format(
            ...
            self.log(
                "CLI templates retrieved: {0}".format(self.pprint(templates)), "INFO"
            )
            return templates

        except Exception as e:
            self.msg = (
                "Error on retrieving templates for profile: Unable to retrieve the templates "
                + "for profile '{0}'".format(profile_id)
            )
            self.log(self.msg + str(e), "ERROR")
            return None

    def attach_networkprofile_cli_template(
        self, profile_name, profile_id, template_name, template_id
    ):
        """
        ...
        """
        self.log(
            "Attaching CLI template '{0}' (ID: {1}) to profile '{2}' (ID: {3})".format(
                template_name, template_id, profile_name, profile_id
            ),
            "INFO",
        )
        function_name = "attach_network_profile_to_a_day_n_cli_template"
        profile_payload = {"profileId": profile_id, "template_id": template_id}

        try:
            return self.execute_process_task_data(
                "configuration_templates", function_name, profile_payload
            )
    ...
    def detach_networkprofile_cli_template(
        self, profile_name, profile_id, template_name, template_id
    ):
        """
        ...
        self.log(
            "Detaching CLI template '{0}' (ID: {1}) from network profile '{2}' (ID: {3})".format(
                template_name, template_id, profile_name, profile_id
            ),
            "INFO",
        )
        function_name = "detach_a_list_of_network_profiles_from_a_day_n_cli_template"
        profile_payload = {"profile_id": profile_id, "template_id": template_id}
        try:
            return self.execute_process_task_data(
                "configuration_templates", function_name, profile_payload
            )
    ...
    def get_site_lists_for_profile(self, profile_name, profile_id):
        ...
        """
        self.log(
            "Fetching site list for profile {0} ID: {1}".format(
        ...
        try:
            ...
            self.log(
                "Response from get site lists for profile API: {0}".format(
                    self.pprint(response)
                ),
                "DEBUG",
            )
            ...
            if not response:
                self.log(
                    "Invalid or missing Site list response, expected list but got {0}".format(
        ...
        if config_type == "template":
            ...
            try:
                    ...
                    if tempaltes:
                        for template in tempaltes:
                            if not self.value_exists(data_list, "name", template):
                                self.log(
                                    "Found Unmatched template: {0}".format(template),
                                    "DEBUG",
                                )
                                un_match_template.append(template)
                            else:
                                self.log(
                                    "Matched template: {0}".format(template), "DEBUG"
                                )
                ...
                if (
                    matched_template
                    and data_list
                    and len(matched_template) <= len(data_list)
                    and not un_match_template
                ):
                    self.log(
                        "Given templates: {0} are matched with existing template: {1}".format(
                            data_list, each_config.get(template_type)
                        ),
                        "DEBUG",
                    )
                ...
                if (
                    not matched_template
                    and not each_config.get("onboarding_templates")
                    and not each_config.get("day_n_templates")
                ):
                    return True, matched_template
            ...
            try:
                if each_config:
                    for site in each_config:
                        self.log(
                            "Checking site: {0} in existing data: {1}.".format(
                                self.pprint(site), self.pprint(data_list)
                            ),
                            "DEBUG",
                        )
                ...
                if un_match_site_ids:
                    self.log(
                        "Found Unmatched site IDs: {0}.".format(
                            self.pprint(un_match_site_ids)
                        ),
                        "DEBUG",
                    )
                ...
                if len(matched_site_ids) <= len(data_list) and not un_match_site_ids:
                    self.log(
                        "Site IDs are matched: {0}.".format(
                            self.pprint(matched_site_ids)
                        ),
                        "DEBUG",
                    )
                    return True, None
            ...
            msg = "compare_config_with_sites_templates: Invalid config_type. Expected 'template' or 'sites', got '{0}'.".format(
                config_type
            )
            self.fail_and_exit(msg)

    def assign_site_to_network_profile(
        self, profile_id, site_id, profile_name, site_name
    ):
        """
        ...
        """
        self.log(
            "Assigning site {0}: {1} to profile {2}:{3}.".format(
                site_name, site_id, profile_name, profile_id
            ),
            "INFO",
        )
        params = {"profile_id": profile_id, "id": site_id}
        ...
        try:
            ...
            error_msg = (
                "Failed to assign the site(s) '{0}' to the profile '{1}'".format(
    ...
    def unassign_site_to_network_profile(
        self, profile_name, profile_id, site_name, site_id
    ):
        """
        ...
        """
        param = {"profile_id": profile_id, "site_id": site_id}
        self.log(
            "Unassigning site {0}: {1} from network profile {2}: {3}.".format(
                site_name, site_id, profile_name, profile_id
            ),
            "INFO",
        )
    ...
    def delete_network_profiles(self, profile_name, profile_id):
        ...
        self.log(
            "Delete network profiles for {0}: {1}".format(profile_name, profile_id),
            "INFO",
        )
        param = {
            "id": profile_id,
    ...
    def execute_process_task_data(
        self, profile_family, profile_function_name, payload_data, task_id=None
    ):
        """
        ...
        try:
            ...
            if not task_id:
                task_id = self.get_taskid_post_api_call(
                    profile_family, profile_function_name, payload_data
                )
                if not task_id:
            ...
            resync_retry_count = int(
                self.payload.get("catalystcenter_api_task_timeout", 10)
            )
            ...
            while resync_retry_count > 0:
                ...
                if not task_details_response:  # Ensure the response is valid
                    self.log(
                        "Failed to retrieve task details for task ID: {0}".format(
                            task_id
                        ),
                        "ERROR",
                    )
                ...
                self.log(
                    "Task ID: {0}, Status: {1}, Attempts remaining: {2}".format(
                        task_id, task_status, resync_retry_count
                    ),
                    "INFO",
                )
            ...
            self.log(
                "Task {0} did not complete within the timeout.".format(task_id), "ERROR"
            )
            return None
    ...
    def deduplicate_list_of_dict(self, list_of_dicts):
        ...
        if not isinstance(list_of_dicts, list):
            self.log(
                "Input is not a list, returning empty list. Input type: {0}".format(
                    type(list_of_dicts).__name__
                ),
                "WARNING",
            )
            return []
        ...
        if not list_of_dicts:
            self.log(
                "Empty list provided for deduplication - returning empty list", "DEBUG"
            )
            return []

        original_count = len(list_of_dicts)
        self.log(
            "Starting deduplication for list with {0} dictionaries: {1}".format(
                original_count, self.pprint(list_of_dicts)
            ),
            "DEBUG",
        )
        ...
        for index, d in enumerate(list_of_dicts):
            ...
            if not isinstance(d, dict):
                self.log(
                    "Skipping non-dictionary item at index {0}: {1} (type: {2})".format(
                        index, d, type(d).__name__
                    ),
                    "WARNING",
                )
                continue
            ...
            try:
                ...
                self.log(
                    "Cannot create hashable identifier for dictionary at index {0}: {1}. Error: {2}".format(
                        index, d, str(e)
                    ),
                    "WARNING",
                )
            ...
            if identifier not in seen:
                ...
                self.log(
                    "Added unique dictionary at index {0} to result list".format(index),
                    "DEBUG",
                )
            else:
                duplicates_found += 1
                self.log(
                    "Found duplicate dictionary at index {0} - skipping".format(index),
                    "DEBUG",
                )
        ...
        if duplicates_found > 0:
            self.log(
                "Deduplication completed: removed {0} duplicate(s) from {1} total items. Final count: {2}".format(
            ...
            self.log(
                "Deduplication completed: no duplicates found in {0} items".format(
                    original_count
                ),
                "DEBUG",
            )
        ...
        self.log(
            "Deduplicated list result: {0}".format(self.pprint(unique_dicts)), "DEBUG"
        )
