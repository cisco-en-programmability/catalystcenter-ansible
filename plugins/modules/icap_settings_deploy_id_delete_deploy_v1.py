...
description:
- Manage operation create of the resource Icap Settings Deploy Id Delete Deploy V1.
- >
   Remove the ICAP configuration from the device by *id* without preview-deploy. The path parameter *id* can be
   retrieved from the **GET /dna/intent/api/v1/icapSettings** API. The response body contains a task object with a
   taskId and a URL. Use the URL to check the task status. ICAP FULL, ONBOARDING, OTA, and SPECTRUM configurations
   have a durationInMins field. A disable task is scheduled to remove the configuration from the device. Removing the
   ICAP intent should be done after the pre-scheduled disable task has been deployed. For detailed information about
...
- name: Cisco DNA Center documentation for Sensors RemoveTheICAPConfigurationOnTheDeviceWithoutPreviewV1
  description: Complete reference of the RemoveTheICAPConfigurationOnTheDeviceWithoutPreviewV1 API.
  link: https://developer.cisco.com/docs/dna-center/#!remove-the-icap-configuration-on-the-device-without-preview
notes:
  - SDK Method used are
    sensors.Sensors.remove_the_i_cap_configuration_on_the_device_without_preview_v1,
...
EXAMPLES = r"""
- name: Create
  cisco.catalystcenter.icap_settings_deploy_id_delete_deploy_v1:
    host: "{{host}}"
    username: "{{username}}"
    password: "{{password}}"
    verify: "{{verify}}"
