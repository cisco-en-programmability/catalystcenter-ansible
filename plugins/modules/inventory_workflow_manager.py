...
class Inventory(DnacBase):
    ...
    def wait_for_device_to_be_managed(self, device_ip, max_retries, retry_interval):
        """
    ...
    def provisioned_wired_device(self):
        ...
        if self.compare_dnac_versions(self.get_ccc_version(), "2.3.5.3") <= 0:
            ...
            for device_info in provision_wired_list:
                ...
                # Check if device reaches managed state
                managed_flag, device_ip_info = self.wait_for_device_to_be_managed(
                    device_ip, resync_retry_count, resync_retry_interval
                )
                if not managed_flag:
    ...
    def provision_wired_device(self, device_ip, site_name_hierarchy, device_type):
        """
    ...
    def get_diff_deleted(self, config):
        ...
        for device_ip in device_to_delete:
            ...
            if self.compare_dnac_versions(self.get_ccc_version(), "2.3.5.3") <= 0:
                self.delete_provisioned_device(device_ip)
                continue
            else:
    ...
    def delete_provisioned_device(self, device_ip):
        """
