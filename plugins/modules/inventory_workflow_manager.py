...
class Inventory(CatalystCenterBase):
    ...
    def get_diff_merged(self, config):
        ...
        if self.config[0].get("role"):
            ...
            if not devices_to_add:
                ...
                self.log(
                    ...
                    "Role will be updated after device addition.".format(
                        str(devices_to_update_role)
                    ),
                    "INFO",
                )
        ...
        if self.config[0].get("add_user_defined_field"):
            ...
            for udf in udf_field_list:
                ...
                check_udf_added_to_device = self.check_udf_added_to_device(
                    device_ids, field_name
                )
    ...
    def check_udf_added_to_device(self, device_ids, udf_field_name):
        ...
        """
        self.log(
            "Checking if UDF '{0}' is already added to the device with ID '{1}'".format(
                udf_field_name, device_ids[0]
            ),
            "DEBUG",
        )
        device_id = device_ids[0]
