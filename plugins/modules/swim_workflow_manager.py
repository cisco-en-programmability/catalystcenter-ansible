...
class Swim(CatalystCenterBase):
    ...
    def get_cco_image_id(self, cco_image_name):
        ...
        try:
            ...
            host = self.params.get("_host")
            self.msg = "CCO image '{0}' not found in the image repository on Cisco Catalyst Center '{1}'".format(cco_image_name, host)
            ...
            if images_to_import:
                ...
                if import_type == "remote":
                    ...
                    for image_name in images_to_import:
                        ...
                        if not cco_image_id:
                            host = self.params.get("_host")
                            self.msg = "CCO image '{0}' not found in the image repository on Cisco Catalyst Center '{1}'".format(image_name, host)
                            self.set_operation_result("failed", False, self.msg, "ERROR").check_return_status()
