...
- >
   Retrieves ICAP configuration deployment statuss per device based on filter criteria. For detailed information
...
- name: Get all Icap Settings Device Deployments V1
  cisco.catalystcenter.icap_settings_device_deployments_v1_info:
    host: "{{host}}"
    username: "{{username}}"
    password: "{{password}}"
    verify: "{{verify}}"
