...
description:
  - Get all Fabric Site Health Summaries Id Trend Analytics.
    - > Get health time series for a specific Fabric
    Site by providing the unique Fabric site id in the
    url path. The data will be grouped based on the
    specified trend time interval. If startTime and
    endTime are not provided, the API defaults to the
    last 24 hours.
...
requirements:
  ...
  - python >= 3.5
seealso:
  - name: Cisco DNA Center documentation for SDA TheTrendAnalyticsDataForAFabricSiteInTheSpecifiedTimeRange
    description: Complete reference of the TheTrendAnalyticsDataForAFabricSiteInTheSpecifiedTimeRange
      API.
    link: https://developer.cisco.com/docs/dna-center/#!the-trend-analytics-data-for-a-fabric-site-in-the-specified-time-range
notes:
  - SDK Method used are
    sda.Sda.the_trend_analytics_data_for_a_fabric_site_in_the_specified_time_range,
  - Paths used are
...
EXAMPLES = r"""
---
- name: Get all Fabric Site Health Summaries Id Trend
    Analytics
