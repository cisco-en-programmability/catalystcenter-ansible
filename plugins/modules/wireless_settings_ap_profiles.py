...
options:
  ...
  awipsForensicEnabled:
    description: Indicates if AWIPS forensic is enabled on the AP. Forensic Capture is supported from IOS-XE version 17.4
      and above. Forensic Capture can be activated only if aWIPS is enabled.
    type: bool
  calendarPowerProfiles:
    ...
    suboptions:
      duration:
        ...
        suboptions:
          schedulerDate:
            description: Start and End date of the duration setting, applicable for MONTHLY schedulers. Values must be between
              1 and 31.
            elements: str
            type: list
          schedulerDay:
            description: Applies every week on the selected days. Ex "sunday","saturday","tuesday","wednesday","thu...
            elements: str
            type: list
          schedulerEndTime:
  ...
  managementSetting:
    ...
    type: dict
  meshEnabled:
    description: This indicates whether mesh networking is enabled on the AP. For IOS-XE devices, when mesh networking is
      enabled, a custom mesh profile with the configured parameters will be created and mapped to the AP join profile on the
      device. When mesh networking is disabled, any existing custom mesh profile will be deleted from the device, and the
  ...
  rogueDetectionSetting:
    ...
    suboptions:
      rogueDetection:
        description: Indicates if rogue detection is enabled on the AP. Detect Access Points that have been installed on a
          secure network without explicit authorization from a system administrator and configure rogue general configuration
          parameters.
        type: bool
