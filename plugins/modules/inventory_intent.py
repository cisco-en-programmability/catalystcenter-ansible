    ...
    def get_diff_deleted(self, config):
        ...
        for device_ip in device_to_delete:
            ...
            try:
                ...
                prov_respone = self.catalystcenter._exec(
                    family="sda",
                    function='get_provisioned_wired_device',
                    op_modifies=True,
...
def main():
    ...
    element_spec = {'_host': {'type': 'str', 'required': True, },
    ...
    catalystcenter_device = DnacDevice(module)
    state = catalystcenter_device.params.get("state")

    if state not in catalystcenter_device.supported_states:
        catalystcenter_device.status = "invalid"
        catalystcenter_device.msg = "State {0} is invalid".format(state)
        catalystcenter_device.check_return_status()

    catalystcenter_device.validate_input().check_return_status()
    config_verify = catalystcenter_device.params.get("config_verify")

    for config in catalystcenter_device.validated_config:
        catalystcenter_device.reset_values()
        catalystcenter_device.get_want(config).check_return_status()
        catalystcenter_device.get_have(config).check_return_status()
        catalystcenter_device.get_diff_state_apply[state](config).check_return_status()
        if config_verify:
            catalystcenter_device.verify_diff_state_apply[state](config).check_return_status()

    module.exit_json(**catalystcenter_device.result)
