  ...
  virtualNetworkName:
    description:
      - VirtualNetworkName query parameter. Name of the virtual network associated
        to the multicast configuration.
    type: str
  offset:
    description:
...
notes:
  - SDK Method used are sda.Sda.get_multicast_virtual_networks_v1,
  - Paths used are get /dna/intent/api/v1/sda/multicast/virtualNetworks,
"""
EXAMPLES = r"""
- name: Get all Sda Multicast Virtual Networks V1
  cisco.catalystcenter.sda_multicast_virtual_networks_v1_info:
    _host: "{{ _host }}"
    _username: "{{ _username }}"
    _password: "{{ _password }}"
    _verify: "{{ _verify }}"
