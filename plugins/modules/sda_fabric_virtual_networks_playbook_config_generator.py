...
options:
  ...
  config:
    ...
    - A dictionary of filters for generating YAML playbook compatible with the
      C(sda_fabric_virtual_networks_workflow_manager) module.
    - Filters specify which components to include in the YAML configuration file.
    - If config is not provided (omitted entirely), all configurations for all Fabric VLANs,
      Virtual Networks, and Anycast Gateways will be generated.
    ...
    suboptions:
      component_specific_filters:
        ...
        - If filters for specific components (for example, C(fabric_vlan), C(virtual_networks),
          or C(anycast_gateways)) are provided without explicitly including them in components_list,
          those components are automatically added to components_list.
...
- |
  ...
  If component-specific filters (such as 'fabric_vlan', 'virtual_networks', or
  'anycast_gateways') are provided without explicitly including them in
  'components_list', those components will be automatically added to
  'components_list'. This simplifies configuration by eliminating the need to
...
seealso:
- module: cisco.catalystcenter.sda_fabric_virtual_networks_workflow_manager
...
- name: Auto-generate YAML Configuration for all components which includes Fabric VLANs, Virtual Networks, and Anycast Gateways
  cisco.catalystcenter.sda_fabric_virtual_networks_playbook_config_generator:
...
class VirtualNetworksPlaybookConfigGenerator(CatalystCenterBase, BrownFieldHelper):
    ...
    def get_fabric_vlans_configuration(self, network_element, filters):
        ...
        if component_specific_filters:
            ...
            for filter_param in component_specific_filters:
                ...
                supported_keys = set(field_map.keys())
                ...
                filtered_vlans.extend(
                    fabric_vlan
                    for fabric_vlan in fabric_vlan_details
                    if all(
                        fabric_vlan.get(field_map[filter_key]) == filter_value
                        for filter_key, filter_value in active_filters.items()
                    )
                )

                if filtered_vlans:
                    final_fabric_vlans.extend(filtered_vlans)
                    self.log(
            ...
            for filter_param in component_specific_filters:
                ...
                if "vn_name" in filter_param:
                    filtered_vns.extend(
                        ...
                        if vn.get("virtualNetworkName") == filter_param["vn_name"]
                    )

                if filtered_vns:
                    final_virtual_networks.extend(filtered_vns)
                    self.log(
    ...
    def get_anycast_gateways_configuration(self, network_element, filters):
        """
        Fetches anycast gateways from the Cisco Catalyst Center using the provided network element and component-specific filters.
        ...
        if component_specific_filters:
            ...
            for filter_param in component_specific_filters:
                ...
                filtered_anycast_gateways.extend(
                    anycast_gateway
                    for anycast_gateway in anycast_gateway_details
                    if all(
                        anycast_gateway.get(field_map[filter_key]) == filter_value
                        for filter_key, filter_value in active_filters.items()
                    )
                )

                if filtered_anycast_gateways:
                    final_anycast_gateways.extend(filtered_anycast_gateways)
                    self.log(
