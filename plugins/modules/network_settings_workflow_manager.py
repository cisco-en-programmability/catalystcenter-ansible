            ...
            "network_management_details": {
                ...
                "settings": {
                    ...
                    "syslog_server": {
                        ...
                        "configure_catalystcenter_ip": {"type": 'bool'}
                    },
                    "snmp_server": {
                        "type": 'dict',
                        "ip_addresses": {"type": 'list'},
        ...
        for item in network_management_details:
            ...
            if self.compare_catalystcenter_versions(self.get_ccc_version(), "2.3.5.3") <= 0:
                ...
                if snmp_server is not None:
                    if snmp_server.get("configure_catalystcenter_ip") is not None:
                        want_network_settings.get("snmpServer").update({
                            "configureDnacIP": snmp_server.get("configure_catalystcenter_ip")
                        })
                ...
                if syslog_server is not None:
                    if syslog_server.get("configure_catalystcenter_ip") is not None:
                        want_network_settings.get("syslogServer").update({
                            "configureDnacIP": syslog_server.get("configure_catalystcenter_ip")
                        })
                ...
                if snmp_server is not None:
                    ...
                    # Check and update configure_catalystcenter_ip
                    configure_catalystcenter_ip = snmp_server.get("configure_catalystcenter_ip")
                    if configure_catalystcenter_ip is not None:
                        self.log("Updating 'useBuiltinTrapServer' with provided configure_catalystcenter_ip: {0}".format(configure_catalystcenter_ip), "INFO")
                        want_network_settings.get("snmpServer").update({
                            "useBuiltinTrapServer": configure_catalystcenter_ip
                        })
                    else:
                ...
                if syslog_server is not None:
                    ...
                    have_configure_catalystcenter_ip = have_syslog_server.get("useBuiltinSyslogServer")
                    have_ip_addresses = have_syslog_server.get("externalSyslogServers")

                    # Update configure_catalystcenter_ip if provided, else fallback to existing value
                    configure_catalystcenter_ip = syslog_server.get("configure_catalystcenter_ip")
                    if configure_catalystcenter_ip is not None:
                        self.log("Updating 'useBuiltinSyslogServer' with provided configure_catalystcenter_ip: {0}".format(configure_catalystcenter_ip), "INFO")
                        want_network_settings.get("syslogServer").update({
                            "useBuiltinSyslogServer": configure_catalystcenter_ip
                        })
                    elif have_configure_catalystcenter_ip is not None:
                        self.log("Retaining existing 'useBuiltinSyslogServer': {0}".format(have_configure_catalystcenter_ip), "INFO")
                        want_network_settings.get("syslogServer").update({
                            "useBuiltinSyslogServer": have_configure_catalystcenter_ip
                        })
                    else:
