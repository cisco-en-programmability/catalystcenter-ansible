...
description:
  - Manage operation update of the resource Sda Fabrics Vlan To Ssids Fabric Id. - > Add, update, or remove SSID mappings
    to a VLAN. If the payload doesn't contain a 'vlanName' which has SSIDs mapping done earlier then all the mapped SSIDs
    of the 'vlanName' is cleared. The request must include all SSIDs currently mapped to a VLAN, as determined by the response
    from the GET operation for the same fabricId used in the request. If an already-mapped SSID is not included in the payload,
    its mapping will be removed by this API. Conversely, if a new SSID is provided, it will be added to the Mapping. Ensure
...
requirements:
  ...
  - python >= 3.12
seealso:
  - name: Cisco DNA Center documentation for Fabric Wireless Add_UpdateOrRemoveSSIDMappingToAVLAN
    description: Complete reference of the Add_UpdateOrRemoveSSIDMappingToAVLAN API.
