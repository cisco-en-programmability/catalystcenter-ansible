...
- name: Generate YAML for all provisioned devices (omit config)
  cisco.catalystcenter.provision_playbook_config_generator:
...
class ProvisionPlaybookGenerator(CatalystCenterBase, BrownFieldHelper):
    """
    ...
    def get_non_provisioned_devices(self, network_element, component_specific_filters=None):
        ...
        try:
            ...
            try:
                provisioned_response = self.catalystcenter._exec(
                    family="sda",
                    function="get_provisioned_devices",
                    op_modifies=False,
                )
...
def main():
    ...
    if (
        ccc_provision_playbook_generator.compare_catalystcenter_versions(
