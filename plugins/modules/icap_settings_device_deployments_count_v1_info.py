...
- >
   Returns the count of device deployment statuss based on filter criteria. For detailed information about the usage
...
- name: Get all Icap Settings Device Deployments Count V1
  cisco.catalystcenter.icap_settings_device_deployments_count_v1_info:
    host: "{{host}}"
    username: "{{username}}"
    password: "{{password}}"
    verify: "{{verify}}"
