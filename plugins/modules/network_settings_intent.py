...
class NetworkSettings(DnacBase):
    ...
    def requires_update(self, have, want, obj_params):
        ...
        Parameters:
            ...
            obj_params (list of tuples) - A list of parameter mappings specifying which
                                          Cisco DNA Center parameters (catalystcenter_param) correspond to
    ...
    def get_want_network(self, network_management_details):
        ...
        if snmpServer is not None:
            if snmpServer.get("configure_catalystcenter_ip") is not None:
                want_network_settings.get("snmpServer").update({
                    "configureDnacIP": snmpServer.get("configure_catalystcenter_ip")
                })
        ...
        if syslogServer is not None:
            if syslogServer.get("configure_catalystcenter_ip") is not None:
                want_network_settings.get("syslogServer").update({
                    "configureDnacIP": syslogServer.get("configure_catalystcenter_ip")
                })
...
def main():
    ...
    catalystcenter_network = NetworkSettings(module)
    state = catalystcenter_network.params.get("state")
    config_verify = catalystcenter_network.params.get("config_verify")
    if state not in catalystcenter_network.supported_states:
        catalystcenter_network.status = "invalid"
        catalystcenter_network.msg = "State {0} is invalid".format(state)
        catalystcenter_network.check_return_status()

    catalystcenter_network.validate_input().check_return_status()

    for config in catalystcenter_network.config:
        catalystcenter_network.reset_values()
        catalystcenter_network.get_have(config).check_return_status()
        if state != "deleted":
            catalystcenter_network.get_want(config).check_return_status()
        catalystcenter_network.get_diff_state_apply[state](config).check_return_status()
        if config_verify:
            catalystcenter_network.verify_diff_state_apply[state](config).check_return_status()

    module.exit_json(**catalystcenter_network.result)
