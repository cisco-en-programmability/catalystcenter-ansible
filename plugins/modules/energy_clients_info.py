...
description:
  - Get all Energy Clients.
  - Get Energy Clients by id.
  - Retrieves a list of client devices with energy data based on the specified query parameters.
  ...
  - Retrieves client device energy data for a specified time range based on the client ID.
  - For detailed information about the usage of the API, please refer to the Open API specification document
    https //github.com/cisco-en-programmability/catalyst-center-api-specs/blob/main/Assurance/
    CE_Cat_Center_Org-deviceEnergy_1.0-1.0.1-resolved.yaml.
...
options:
  ...
  cursor:
    description:
      - >
        Cursor query parameter. It's an opaque string field that indicates the next record in the requested
        collection. If no records remain, the API returns a response with a count of zero. The default value is
        an empty string, and the initial value must be an empty string. The cursor value is populated by the API
        in the response page block. If the user wants more records, the cursor in the subsequent request must be
  ...
  id:
    description:
      - >
        Id query parameter. The list of Mac addresses (e.g., `54 9F C6 43 FF 80`). Examples `id=54 9F C6 43 FF
        80` (single device requested) `id=54 9F C6 43 FF 80&id=01 23 45 67 89 AB`.
    type: str
  siteId:
    description:
      - >
  ...
  deviceSubCategory:
    description:
      - >
        DeviceSubCategory query parameter. The list of device sub categories. Examples `deviceSubCategory=IP
  ...
  attribute:
    description:
      - >
        Attribute query parameter. List of attributes. Please refer to `ClientDeviceEnergyAttribute` for
...
requirements:
  ...
  - python >= 3.12
seealso:
  - name: Cisco DNA Center documentation for Clients GetClientEnergyByID
    description: Complete reference of the GetClientEnergyByID API.
    link: https://developer.cisco.com/docs/dna-center/#!get-client-energy-by-id
  - name: Cisco DNA Center documentation for Clients GetClientsEnergy
    description: Complete reference of the GetClientsEnergy API.
...
- name: Get all Energy Clients
  cisco.catalystcenter.energy_clients_info:
