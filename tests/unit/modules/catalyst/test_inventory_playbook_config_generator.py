...
class TestBrownfieldInventoryPlaybookGenerator(TestDnacModule):
    ...
    def test_inventory_playbook_config_generator_scenario1_complete_infrastructure(self):
        ...
        set_module_args(
            dict(
                ...
                catalystcenter_version="2.3.3.0",
    ...
    def test_inventory_playbook_config_generator_dnac_connection_failure(self):
        ...
        to Cisco Catalyst Center and returns appropriate error messages.
        """
        self.run_dnac_init.side_effect = Exception("Unable to connect to Cisco Catalyst Center")

        set_module_args(
            dict(
                catalystcenter_host="invalid.host.example.com",
                catalystcenter_username="admin",
                catalystcenter_password="admin123",
                catalystcenter_verify=False,
                state="gathered",
        ...
        result = self.execute_module(changed=False, failed=True)
        self.assertIn(
            "Unable to connect to Cisco Catalyst Center",
            result.get('msg', '')
        )
