...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/tags_manager/playbook/tags_manager_playbook.yml -vvvv
```
...
```bash
  ...
  -s cvp/tags_manager/schema/tags_manager_schema.yml \
  -v cvp/tags_manager/vars/tags_manager_inputs.yml
```

Return result validate:
```code
  (pyats-ansible-phamdat) bash-4.4$ ./tools/schemavalidation.sh -s cvp/tags_manager/schema/tags_manager_schema.yml -v cvp/tags_manager/vars/tags_manager_inputs.yml
  cvp/tags_manager/schema/tags_manager_schema.yml
  cvp/tags_manager/vars/tags_manager_inputs.yml
  yamale   -s cvp/tags_manager/schema/tags_manager_schema.yml  cvp/tags_manager/vars/tags_manager_inputs.yml
  Validating cvp/tags_manager/vars/tags_manager_inputs.yml...
  Validation success! 👍
...
```bash
  ansible-playbook \
    ...
    cvp/tags_manager/playbook/tags_manager_playbook.yml \
...
2. Copy the sample workflow data from `cvp/tags_manager/vars/tags_manager_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `tags_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/tags_manager/playbook/tags_manager_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/tags_manager/vars/tags_manager_inputs.yml`
