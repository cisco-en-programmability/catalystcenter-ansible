...
# How to run
  ...
  ansible-playbook 
    ...
    ./cvp/swim/playbook/swim_workflow_playbook.yml # playbook will run this
  ...
  ansible-playbook 
    -i ./inventory/demo_lab/hosts.yml # refer to Catalyst Center to run
    ./cvp/swim/playbook/delete_swim_workflow_playbook.yml # playbook will run this
  ...
  ```bash
    ...
    -s cvp/swim/schema/swim_schema.yml \
    -v cvp/swim/vars/swim_import_tag_distribute_activate_image_vars.yml
  ```

  ### b. Include delete images (state = 'deleted')
  ```bash
    ./tools/schemavalidation.sh \
    -s cvp/swim/schema/delete_swim_schema.yml \
    -v cvp/swim/vars/delete_swim_vars.yml
  ```
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/swim/playbook/swim_workflow_playbook.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/swim/vars/swim_import_tag_distribute_activate_image_vars.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `swim_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/swim/playbook/swim_workflow_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/swim/vars/swim_import_tag_distribute_activate_image_vars.yml`
