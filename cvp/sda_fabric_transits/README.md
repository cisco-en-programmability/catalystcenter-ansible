...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/sda_fabric_transits/playbook/sda_fabric_transits_workflow_playbook.yml -vvvv
```
...
Create a variable file (e.g., `cvp/sda_fabric_transits/vars/sda_fabric_transits_workflow_inputs.yml`) to specify the desired state of your fabric transits for creation, update, or deletion.
...
```bash
yamale -s cvp/sda_fabric_transits/schema/sda_fabric_transits_workflow_schema.yml cvp/sda_fabric_transits/vars/sda_fabric_transits_workflow_inputs.yml
```
...
1. **Deploy Configuration (Create/Update)**:  
   Run the playbook to apply the fabric transit configuration. Ensure the input file is validated before execution. Specify the input file path using `VARS_FILE_PATH`. Because Ansible resolves `VARS_FILE_PATH` relative to the playbook directory, use `../vars/<file>.yml` or `${PWD}/cvp/.../vars/<file>.yml`.  
   ```bash
   ansible-playbook -i inventory/demo_lab/hosts.yaml cvp/sda_fabric_transits/playbook/sda_fabric_transits_workflow_playbook.yml --e VARS_FILE_PATH=../vars/sda_fabric_transits_workflow_inputs.yml > logs/transits.log -vvvvvv
   ```
...
2. **Deploy Configuration (Delete)**:  
   ...
   ```bash
   ansible-playbook -i inventory/demo_lab/hosts.yaml cvp/sda_fabric_transits/playbook/delete_sda_fabric_transits_workflow_playbook.yml --e VARS_FILE_PATH=../vars/sda_fabric_transits_workflow_inputs.yml > logs/transits_delete.log -vvvvvv
   ```
...
2. Copy the sample workflow data from `cvp/sda_fabric_transits/vars/sda_fabric_transits_workflow_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `fabric_transits`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/sda_fabric_transits/playbook/sda_fabric_transits_workflow_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/sda_fabric_transits/vars/sda_fabric_transits_workflow_inputs.yml`
