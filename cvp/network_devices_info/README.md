...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/network_devices_info/playbook/network_devices_info_playbook.yml -vvvv
```
...
```bash
yamale -s cvp/network_devices_info/schema/network_devices_info_schema.yml        cvp/network_devices_info/vars/network_devices_info_input.yml
```
Return result validate:

```bash
(pyats-nalakkam) [nalakkam@st-ds-4 dnac_ansible_workflows]$ yamale -s cvp/network_devices_info/schema/network_devices_info_schema.yml        cvp/network_devices_info/vars/network_devices_info_input.yml
Validating cvp/network_devices_info/vars/network_devices_info_input.yml...
Validation success! 👍
```
...
ansible-playbook -i inventory/demo_lab/hosts.yaml \
  cvp/network_devices_info/playbook/network_devices_info_playbook.yml \
...
2. Review the [Catalyst Center API Documentation](https://developer.cisco.com/docs/catalyst-center/)
...
2. Copy the sample workflow data from `cvp/network_devices_info/vars/network_devices_info_input.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `network_devices_info_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/network_devices_info/playbook/network_devices_info_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/network_devices_info/vars/network_devices_info_input.yml`
