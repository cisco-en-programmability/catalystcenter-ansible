...
ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/network_settings/playbook/network_settings_playbook.yml --e VARS_FILE_PATH=../vars/network_settings_vars.yml -vvv 

## Example run: Create IP Pools and Reserve IP pools on Sites
ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/network_settings/playbook/network_settings_playbook.yml --e VARS_FILE_PATH=../vars/global_pool_and_reserve_pools_on_sites.yml -vvv 

## Updating servers on sites AAA NTP, DNS, DHCP, TimeZone, SNMP, Logging, Banner etc.
ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/network_settings/playbook/network_settings_playbook.yml --e VARS_FILE_PATH=../vars/server_update_aaa_ntp_dns_dhcp_tz_banner_syslog_snmp_netflow.yml -vvv 

##Example run: Delete Network Settings

ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/network_settings/playbook/delete_network_settings_playbook.yml --e VARS_FILE_PATH=../vars/network_settings_vars.yml -vvv 
...
```bash
(pyats) pawansi@PAWANSI-M-81A3 dnac_ansible_workflows % ./tools/schemavalidation.sh -s cvp/network_settings/schema/nw_settings_schema.yml -v cvp/network_settings/vars/network_settings_vars.yml
cvp/network_settings/schema/nw_settings_schema.yml
cvp/network_settings/vars/network_settings_vars.yml
yamale   -s cvp/network_settings/schema/nw_settings_schema.yml  cvp/network_settings/vars/network_settings_vars.yml
Validating /Users/pawansi/dnac_ansible_cvp/cvp/network_settings/vars/network_settings_vars.yml...
Validation success! 👍
```
...
### 4. How to Run
  ...
  **For the Deleted Method**, change the destination playbook to: `./cvp/network_settings/playbook/delete_network_settings_playbook.yml`
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/network_settings/playbook/network_settings_playbook.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/network_settings/vars/aaa_servers_vars.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `network_settings_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/network_settings/playbook/network_settings_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/network_settings/vars/aaa_servers_vars.yml`
