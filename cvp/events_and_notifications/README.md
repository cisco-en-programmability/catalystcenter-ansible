...
- **Cisco Catalyst Center or Events and Notifications Connect Access**: Ensure access is configured

## Configure Environment
- Update hosts.yml with the connection details of your Catalyst Center instance. 
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/events_and_notifications/playbook/events_and_notifications_playbook.yml -vvvv
```
...
```bash
ansible-playbook -i ./inventory/demo_lab/inventory_demo_lab.yml ./cvp/events_and_notifications/playbook/events_and_notifications_playbook.yml --extra-vars VARS_FILE_PATH=../vars/events_and_notifications_destinations_inputs.yml -vvvv
```
...
```bash
ansible-playbook -i ./inventory/demo_lab/inventory_demo_lab.yml ./cvp/events_and_notifications/playbook/delete_events_and_notifications_playbook.yml --extra-vars VARS_FILE_PATH=../vars/events_and_notifications_destinations_inputs.yml -vvvv
```
...
Playbook: cvp/events_and_notifications/playbook/events_and_notifications_playbook.yml
Inputs: cvp/events_and_notifications/vars/events_and_notifications_destinations_inputs.yml
* Use `yamale`:

```bash
yamale -s cvp/events_and_notifications/schema/events_and_notifications_schema.yml cvp/events_and_notifications/vars/events_and_notifications_destinations_inputs.yml 
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/events_and_notifications/playbook/events_and_notifications_playbook.yml --e VARS_FILE_PATH=../vars/events_and_notifications_destinations_inputs.yml
```
...
2. Copy the sample workflow data from `cvp/events_and_notifications/vars/events_and_notifications_destinations_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `events_notifications_destination_and_subscription_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/events_and_notifications/playbook/events_and_notifications_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/events_and_notifications/vars/events_and_notifications_destinations_inputs.yml`
