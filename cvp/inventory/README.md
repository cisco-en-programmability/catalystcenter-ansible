...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/inventory/playbook/inventory_playbook.yml -vvvv
```
...
```bash
yamale -s cvp/inventory/schema/inventory_schema.yml cvp/inventory/vars/inventory_vars.yml 
Validating /Users/pawansi/dnac_ansible_cvp/cvp/inventory/vars/inventory_vars.yml...
Validation success! 👍
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/inventory/playbook/inventory_playbook.yml --e VARS_FILE_PATH=../vars/inventory_vars.yml -vvvvv
```

B. To execute the Ansible playbook for provision devices:

* This task provisions wired devices in the Cisco Catalyst Center inventory. Provisioning involves assigning devices to a specific site and applying the necessary configurations for them to operate within that site's network environment.
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/inventory/playbook/inventory_playbook.yml --e VARS_FILE_PATH=../vars/inventory_provision_devices.yml -vvvvv
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/inventory/playbook/inventory_playbook.yml --e VARS_FILE_PATH=../vars/inventory_resync_reboot_vars.yml -vvvvv
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/inventory/playbook/delete_inventory_playbook.yml --e VARS_FILE_PATH=../vars/inventory_delete_devices.yml -vvvvv
```
...
2. Copy the sample workflow data from `cvp/inventory/vars/inventory_provision_devices.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `inventory_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/inventory/playbook/inventory_playbook.yml -vvvv
```
