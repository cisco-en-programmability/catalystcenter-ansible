...
Ansible playbook / workflow for managing host onboarding (wired port assignments, port channels, and Fabric Wireless SSID ↔ VLAN mappings) in a Cisco SD‑Access Fabric through Cisco Catalyst Center (Catalyst Center).  
...
- Clone repository: `git@github.com:cisco-en-programmability/catalystcenter-ansible.git`
...
```bash
yamale -s schema/sda_host_onboarding_schema.yml cvp/sda_hostonboarding/vars/sda_host_onboarding_input.yml
```
Output:
```bash
Validation success! 👍
...
ansible-playbook -i host_inventory_dnac1/hosts.yml \
  cvp/sda_hostonboarding/playbook/sda_host_onboarding_playbook.yml \
...
```bash
ansible-playbook -i host_inventory_dnac1/hosts.yml \
  cvp/sda_hostonboarding/playbook/delete_sda_host_onboarding_playbook.yml \
  --e VARS_FILE_PATH=../vars/sda_host_onboarding_input.yml -vvvv
```
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/sda_hostonboarding/playbook/sda_host_onboarding_playbook.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/sda_hostonboarding/vars/sda_host_onboarding_input.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `sda_host_onboarding_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/sda_hostonboarding/playbook/sda_host_onboarding_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/sda_hostonboarding/vars/sda_host_onboarding_input.yml`
