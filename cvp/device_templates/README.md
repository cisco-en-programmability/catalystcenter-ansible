...
```bash
yamale -s cvp/device_templates/schema/template_workflow_schema.yml cvp/device_templates/vars/template_workflow_inputs.yml 
```
```
Validating /Users/pawansi/dnac_ansible_cvp/cvp/device_templates/vars/template_workflow_inputs.yml...
Validation success! 👍
```
...
```bash
    ansible-playbook -i inventory/demo_lab/hosts.yaml cvp/device_templates/playbook/template_workflow_playbook.yml --e VARS_FILE_PATH=../vars/template_workflow_inputs.yml
```

*  How to Delete Existing Devices from inventory.
*  Run the Delete Playbook:
```bash
    ansible-playbook -i inventory/demo_lab/hosts.yaml cvp/device_templates/playbook/delete_template_workflow_playbook.yml --e VARS_FILE_PATH=../vars/delete_template_workflow_inputs.yml
```
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/device_templates/playbook/template_workflow_playbook.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/device_templates/vars/template_workflow_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `template_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/device_templates/playbook/template_workflow_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/device_templates/vars/template_workflow_inputs.yml`
