...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/network_profile_wireless/playbook/network_profile_wireless_playbook.yml -vvvv
```
...
```bash
./tools/schemavalidation.sh -s ./cvp/network_profile_wireless/schema/network_profile_wireless_schema.yml -v ./cvp/network_profile_wireless/vars/network_profile_wireless_inputs.yml
```
---
...
1. **Deploy Configuration**:  
   Run the playbook to apply the wireless network profile configuration. Ensure the input file is validated before execution. Specify the input file path using `VARS_FILE_PATH`. Because Ansible resolves `VARS_FILE_PATH` relative to the playbook directory, use `../vars/<file>.yml` or `${PWD}/cvp/.../vars/<file>.yml`.

  ### a. Include create/update network profile (state = 'merged')
   ```bash
  ansible-playbook -i inventory/demo_lab/hosts.yaml \
  cvp/network_profile_wireless/playbook/network_profile_wireless_playbook.yml \
  ...
  ansible-playbook 
    ...
    cvp/network_profile_wireless/playbook/delete_network_profile_wireless_playbook.yml # playbook will run this
...
2. Copy the sample workflow data from `cvp/network_profile_wireless/vars/network_profile_wireless_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `wireless_nw_profiles_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/network_profile_wireless/playbook/network_profile_wireless_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/network_profile_wireless/vars/network_profile_wireless_inputs.yml`
