...
>git@github.com:cisco-en-programmability/catalystcenter-ansible.git
...
```bash
yamale -s cvp/device_credentials/schema/device_credentials_schema.yml cvp/device_credentials/vars/device_credentials_vars.yml
```

- Result:
```bash
Validating /Users/pawansi/dnac_ansible_cvp/cvp/device_credentials/vars/device_credentials_vars.yml...
Validation success! 👍
```
...
```bash
ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/device_credentials/playbook/device_credentials_playbook.yml --e VARS_FILE_PATH=../vars/device_credentials_vars.yml -vvvv
```
- Result:

```bash
TASK [Create or Update existing Credentials with provided details in "../vars/device_credentials_vars.yml"] ***
task path: /auto/dna-sol/ws/thanduong/dnac-auto/dnac_ansible_cvp/cvp/device_credentials/playbook/device_credentials_playbook.yml:53
...
task path: /auto/dna-sol/ws/thanduong/dnac-auto/dnac_ansible_cvp/cvp/device_credentials/playbook/device_credentials_playbook.yml:61
<catalyst_center220> ESTABLISH LOCAL CONNECTION FOR USER: thanduon
<catalyst_center220> EXEC /bin/sh -c 'echo ~thanduon && sleep 0'
...
```bash
ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/device_credentials/playbook/delete_device_credentials_playbook.yml --e VARS_FILE_PATH=../vars/device_credentials_vars.yml -vvvv
```
- Reult:

```bash
TASK [Create or Update existing Credentials with provided details in "../vars/device_credentials_vars.yml"] ***
task path: /auto/dna-sol/ws/thanduong/dnac-auto/dnac_ansible_cvp/cvp/device_credentials/playbook/delete_device_credentials_playbook.yml:36
<catalyst_center220> ESTABLISH LOCAL CONNECTION FOR USER: thanduon
<catalyst_center220> EXEC /bin/sh -c 'echo ~thanduon && sleep 0'
...
Defining the jinja template example: cvp/device_credentials/jinja_template/device_credentials_template.j2
...
```bash
ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/device_credentials/playbook/device_credentials_playbook.yml --e VARS_FILE_PATH=../vars/jinja_template_device_credentials_input.yml -vvvv
```
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/device_credentials/playbook/device_credentials_playbook.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/device_credentials/vars/device_credentials_vars.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `device_credentials`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/device_credentials/playbook/device_credentials_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/device_credentials/vars/device_credentials_vars.yml`
