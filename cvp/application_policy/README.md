...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/application_policy/playbook/application_policy_playbook.yml -vvvv
```
...
```bash
  yamale -s cvp/application_policy/schema/application_policy_schema.yml cvp/application_policy/vars/application_policy_inputs.yml
```

result:

```
  Validating cvp/application_policy/vars/application_policy_inputs.yml...
  Validation success! 👍
```
...
```bash
  ansible-playbook -i inventory/iac/host.yml cvp/application_policy/playbook/application_policy_playbook.yml --e VARS_FILE_PATH=../vars/application_policy_inputs.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/application_policy/vars/application_policy_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `application_policy_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/application_policy/playbook/application_policy_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/application_policy/vars/application_policy_inputs.yml`
