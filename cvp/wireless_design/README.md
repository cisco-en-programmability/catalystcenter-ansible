...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/wireless_design/playbook/wireless_design_playbook.yml -vvvv
```
...
```bash
  ...
  -s cvp/wireless_design/schema/wireless_design_schema.yml \
  -v cvp/wireless_design/vars/wireless_design_inputs.yml
```

Return result validate:
```code
  (pyats-ansible-phamdat) bash-4.4$ ./tools/schemavalidation.sh -s cvp/wireless_design/schema/wireless_design_schema.yml -v cvp/wireless_design/vars/wireless_design_inputs.yml 
  cvp/wireless_design/schema/wireless_design_schema.yml
  cvp/wireless_design/vars/wireless_design_inputs.yml
  yamale   -s cvp/wireless_design/schema/wireless_design_schema.yml  cvp/wireless_design/vars/wireless_design_inputs.yml
  Validating cvp/wireless_design/vars/wireless_design_inputs.yml...
  Validation success! 👍
...
```bash
  ansible-playbook \
    ...
    cvp/wireless_design/playbook/wireless_design_playbook.yml \
...
```bash
  ansible-playbook \
    -i inventory/demo_lab/hosts.yaml \
    cvp/wireless_design/playbook/delete_wireless_design_playbook.yml \
    --e VARS_FILE_PATH=../vars/wireless_design_inputs.yml \
    -vvv
``` 
...
```bash
  ansible-playbook -i inventory/demo_lab/hosts.yaml cvp/wireless_design/playbook/wireless_design_playbook.yml --extra-vars VARS_FILE_PATH=../vars/jinja_wireless_design_inputs.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/wireless_design/vars/wireless_design_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `wireless_design_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/wireless_design/playbook/wireless_design_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/wireless_design/vars/wireless_design_inputs.yml`
