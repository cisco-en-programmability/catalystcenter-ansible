...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/sda_fabric_sites_zones/playbook/sda_fabric_sites_zones_playbook.yml -vvvv
```
...
```bash
   yamale -s cvp/sda_fabric_sites_zones/schema/sda_fabric_sites_zones_schema.yml cvp/sda_fabric_sites_zones/vars/sda_fabric_sites_zones_inputs.yml
```

result:

```
    Validating /auto/dna-sol/ws/thanduong/dnac-auto/dnac_ansible_cvp/cvp/sda_fabric_sites_zones/vars/sda_fabric_sites_zones_inputs.yml...
    Validation success! 👍
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/sda_fabric_sites_zones/playbook/sda_fabric_sites_zones_playbook.yml --e VARS_FILE_PATH=../vars/sda_j2_template_fabric_sites_input.yml
...
2. Copy the sample workflow data from `cvp/sda_fabric_sites_zones/vars/sda_fabric_sites_zones_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `fabric_sites_and_zones`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/sda_fabric_sites_zones/playbook/sda_fabric_sites_zones_playbook.yml -vvvv
```
...
- Fully resolved from the repo root: `${PWD}/cvp/sda_fabric_sites_zones/vars/sda_fabric_sites_zones_inputs.yml`
