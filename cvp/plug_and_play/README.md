...
- **Cisco Catalyst Center or Plug and Play Connect Access**: Ensure access is configured
...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/plug_and_play/playbook/catalyst_center_pnp_playbook.yml -vvvv
```
...
```bash
  ...
  -s cvp/plug_and_play/schema/plug_and_play_schema.yml \
  -v cvp/plug_and_play/vars/catalyst_center_pnp_vars.yml
```

**Return result validate**
```
Validating cvp/plug_and_play/vars/catalyst_center_pnp_vars.yml...
Validation success! 👍
```
...
```bash
ansible-playbook -i ./inventory/demo_lab/inventory_demo_lab.yml ./cvp/plug_and_play/playbook/catalyst_center_pnp_playbook.yml --extra-vars VARS_FILE_PATH=../vars/catalyst_center_pnp_vars.yml -vvvv
```
...
2. Copy the sample workflow data from `cvp/plug_and_play/vars/catalyst_center_pnp_vars.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `pnp_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/plug_and_play/playbook/catalyst_center_pnp_playbook.yml -vvvv
```
