...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/site_hierarchy/playbook/site_hierarchy_playbook.yml -vvvv
```
...
1. **Define Input Variables:**  
   Create the variable file (e.g., `cvp/site_hierarchy/vars/site_hierarchy_design_vars.yml`). This file should contain the list of sites (areas, buildings, floors) you want to configure.
...
4. Execute: Execute the playbook with your inputs and inventory. Specify your input file using `VARS_FILE_PATH`. Because Ansible resolves it relative to the playbook directory, use `../vars/<file>.yml` or `${PWD}/cvp/.../vars/<file>.yml`.
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/site_hierarchy/playbook/site_hierarchy_playbook.yml --e VARS_FILE_PATH=/Users/pawansi/dnac_ansible_cvp/cvp/site_hierarchy/vars/site_hierarchy_design_vars.yml -vvv
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/site_hierarchy/playbook/site_hierarchy_playbook.yml --e VARS_FILE_PATH=/Users/pawansi/dnac_ansible_cvp/cvp/site_hierarchy/vars/jinja_template_site_hierarchy_design_vars.yml -vvv
```
...
```bash
    ansible-playbook -i host_inventory_dnac1/hosts.yml cvp/site_hierarachy/playbook/delete_site_hierarchy_playbook.yml --e VARS_FILE_PATH=/Users/pawansi/dnac_ansible_cvp/cvp/site_hierarchy/vars/delete_site_hierarchy_design_vars.yml -vvv
```
...
```bash
yamale -s cvp/site_hierarchy/schema/sites_schema.yml cvp/site_hierarchy/vars/site_hierarchy_design_vars.yml
```
Validating cvp/site_hierarchy/vars/site_hierarchy_design_vars.yml...  
Validation success! 👍
...
```bash
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./cvp/site_hierarchy/playbook/site_hierarchy_playbook.yml --extra-vars VARS_FILE_PATH=../vars/site_hierarchy_design_vars.yml -vvvvvv
```
...
2. Copy the sample workflow data from `cvp/site_hierarchy/vars/site_hierarchy_design_vars.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `design_sites`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> cvp/site_hierarchy/playbook/site_hierarchy_playbook.yml -vvvv
```
